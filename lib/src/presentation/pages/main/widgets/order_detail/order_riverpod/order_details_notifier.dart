...
import 'package:garista_pos/src/core/utils/app_connectivity.dart';
...
class OrderDetailsNotifier extends StateNotifier<OrderDetailsState> {
  ...
  void toggleOrderDetailChecked({required int index}) {
    List<Dish>? orderDetails = state.order?.dishes;
    if (orderDetails == null || orderDetails.isEmpty) {
      return;
    }
    Dish detail = orderDetails[index];
    // final bool isChecked = detail.visibility ?? false;
  ...
  Future<void> fetchOrderDetails({OrderData? order}) async {
    ...
    response.when(
      success: (data) {
        ...
        print("The Order Detail Lenght => ${data.data?.dishes?.length}");
      },
      failure: (failure) {
