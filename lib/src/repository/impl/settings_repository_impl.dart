...
import 'package:garista_pos/src/models/response/sale_cart_response.dart';
...
class SettingsRepositoryImpl extends SettingsRepository {
  ...
  Future<ApiResult<SaleHistoryResponse>> getSaleHistory(
    ...
    try {
      ...
      final response = await client.get(
        '/api/v1/dashboard/${LocalStorage.getUser()?.id}/sales-history',
        queryParameters: data,
      );
      return ApiResult.success(
  ...
  Future<ApiResult<SaleCartResponse>> getSaleCart() async {
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.get(
        '/api/v1/dashboard/${LocalStorage.getUser()?.id}/sales-cards',
      );
      return ApiResult.success(
