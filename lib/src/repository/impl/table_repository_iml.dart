...
import 'package:garista_pos/src/models/response/close_day_response.dart';
import 'package:garista_pos/src/models/response/table_info_response.dart';
import 'package:garista_pos/src/models/response/table_statistic_response.dart';
import 'package:garista_pos/src/models/response/working_day_response.dart';
...
class TableRepositoryIml extends TableRepository {
  ...
  Future<ApiResult<TableBookingResponse>> getTableOrders({
    ...
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.get(
        '/api/v1/dashboard/${LocalStorage.getUser()?.id}/user-bookings',
        queryParameters: data,
      );
      return ApiResult.success(
  ...
  Future<ApiResult<TableResponse>> deleteSection(int id) async {
    try {
      ...
      final response = await client.delete(
        '/api/v1/dashboard/${LocalStorage.getUser()?.id}/shop-sections/delete',
  ...
  Future<ApiResult<TableResponse>> deleteTable(int id) async {
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.delete(
        '/api/v1/dashboard/${LocalStorage.getUser()?.id}/tables/delete',
        queryParameters: {"ids[0]": id},
      );
      return ApiResult.success(
        data: TableResponse.fromJson(response.data),
  ...
  Future<ApiResult<List<DisableDates>>> disableDates({
    ...
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.get(
        '/api/v1/dashboard/${LocalStorage.getUser()?.id}/disable-dates/table/$id',
        queryParameters: {
          'lang': LocalStorage.getLanguage()?.locale ?? 'en',
  ...
  Future<ApiResult<BookingsResponse>> getBookings({int? page}) async {
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.get(
        '/api/v1/dashboard/${LocalStorage.getUser()?.id}/bookings',
        queryParameters: {
          'lang': LocalStorage.getLanguage()?.locale ?? 'en',
          'page': page,
          'perPage': 100
  ...
  Future<ApiResult<WorkingDayResponse>> getWorkingDay() async {
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.get(
        '/api/v1/dashboard/${LocalStorage.getUser()?.id}/booking/shop-working-days/${LocalStorage.getUser()?.id == TrKeys.waiter ? LocalStorage.getUser()?.id : LocalStorage.getUser()?.id}',
  ...
  Future<ApiResult<CloseDayResponse>> getCloseDay() async {
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.get(
        '/api/v1/dashboard/${LocalStorage.getUser()?.id}/booking/shop-closed-dates/${LocalStorage.getUser()?.id == TrKeys.waiter ? LocalStorage.getUser()?.id : LocalStorage.getUser()?.id}',
        queryParameters: {'lang': LocalStorage.getLanguage()?.locale ?? 'en'},
      );
  ...
  Future<ApiResult<TableInfoResponse>> getTableInfo(int id) async {
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.get(
        '/api/v1/dashboard/${LocalStorage.getUser()?.id}/user-bookings/$id',
        queryParameters: {'lang': LocalStorage.getLanguage()?.locale ?? 'en'},
      );
  ...
  Future<ApiResult> changeOrderStatus(
    ...
    try {
      final client = dioHttp.client(requireAuth: true);
      await client.post(
        '/api/v1/dashboard/${LocalStorage.getUser()?.id}/user-booking/status/$id',
  ...
  Future<ApiResult<TableStatisticResponse>> getStatistic({
    ...
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.get(
          '/api/v1/dashboard/${LocalStorage.getUser()?.id}/table/statistic',
          queryParameters: {
