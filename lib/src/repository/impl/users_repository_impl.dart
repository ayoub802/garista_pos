...
// import 'package:garista_pos/src/core/routes/app_router.gr.dart';
import 'package:garista_pos/src/models/data/customer_model.dart';
import 'package:garista_pos/src/models/response/edit_profile.dart';
import 'package:garista_pos/src/models/response/profile_response.dart';
...
class UsersRepositoryImpl extends UsersRepository {
  ...
  Future<ApiResult<SingleUserResponse>> getUserDetails(String uuid) async {
    try {
      ...
      final response = await client.get(
        '/api/users/${LocalStorage.getUser()?.id}',
      );
      return ApiResult.success(
  ...
  Future<ApiResult<ProfileResponse>> getProfileDetails(
    ...
    try {
      final client =
          dioHttp.client(requireAuth: true, baseUrl: SecretVars.GaristabaseUrl);
      final response = await client.get(
        '/api/users/${LocalStorage.getUser()?.id}', // Replace with the user ID dynamically
      );
      return ApiResult.success(
  ...
  Future<ApiResult<void>> updateDeliveryZones({
    ...
    final data = {
      'shop_id': LocalStorage.getUser()?.id,
      'address': tapped,
    };
  ...
  Future<ApiResult<DeliveryZonePaginate>> getDeliveryZone() async {
    final int? shopID = LocalStorage.getUser()?.id;
    final data = {
      'lang': LocalStorage.getLanguage()?.locale ?? 'en',
    };
    try {
  ...
  Future<ApiResult<ProfileResponse>> createUser(
    ...
    try {
      final client = dioHttp.client(requireAuth: true);
      final response = await client.post(
        '/api/v1/dashboard/${LocalStorage.getUser()?.id}/users',
        data: data,
      );
      return ApiResult.success(
        data: ProfileResponse.fromJson(response.data),
  ...
  Future<ApiResult<UsersPaginateResponse>> getUsers({int? page}) async {
    ...
    try {
      final client = dioHttp.client(requireAuth: true);
      final response =
          await client.get('/api/users/${LocalStorage.getUser()?.id}');
      return ApiResult.success(
        data: UsersPaginateResponse.fromJson(response.data),
      );
    } catch (e) {
